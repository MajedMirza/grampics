...
class Account
{
    ...
    public function register_user($username, $fullName, $email, $password)
    {
        ...
        $stmt = $this->pdo->prepare("INSERT INTO user (username,fullName,password,email,pri_ip,pri_os,pri_browser) VALUES (:username,:fullName,:password,:email,:ip,:os,:browser)");
    ...
    public function login_user($email_username, $password)
    {
        if(!empty($email_username) &&!empty($password)) {
            $stmt = $this->pdo->prepare("SELECT * FROM user WHERE email = :username OR username = :username");
        ...
        if ($stmt->rowCount() != 0) {
            if (password_verify($password, $row->password)) {
                ...
                $this->addError("Username and Password do not match");
                return false;
            }
        } else {
            $this->addError("Username and Password do not match");
    ...
    public function getBrowser()
    {
        ...
        $browser_array = array(
            ...
            '#opera#i' => 'Opera',
            '#netscape#i' => 'Netscape',
            '#maxthon#i' => 'Maxthon', '#konqueror#i' => 'Konqueror', '#mobile#i' => 'Handheld Browser',
        );
    ...
    {
        $stmt = $this->pdo->prepare("SELECT * FROM user WHERE $item=:$item");
