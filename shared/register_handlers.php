...
if (Input::exists()) {
    if (isset($_POST['submitButton'])) {
        ...
        //Call the function to check minimum required length and merge return data into form_error array
        $form_errors = array_merge($form_errors, check_min_length($fields_to_check_length));
        //Email validation / merge the return data into form_error array
        $form_errors = array_merge($form_errors, check_email($_POST));

        $rules = [
            'email' => array('unique' => 'user'),
            'username' => array('unique' => 'user'),
        ...
        if ($account->passed()) {
            //Checks if error array is empty, if yes processes data and insert record
